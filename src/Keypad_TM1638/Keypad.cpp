...
void Keypad::begin(char *userKeymap) {
	...
	//Serial.println("tm.displayBegin");
	tm.displayBegin(); // Init the module
	tm.DisplayStr(" hello ", 0); 
	delay(500);
	tm.DisplayStr("       ", 0); 
}
...
void Keypad::scanKeys() {
	...
	*/
	//buttons = tm.ReadKey16();// 1 seul bouton
	buttons = tm.ReadKey16Two();// plusieurs boutons
	// bitMap stores ALL the keys that are being pressed.
	 for (byte c=0; c<sizeKpd.columns; c++) {
		for (byte r=0; r<sizeKpd.rows; r++) {
		  Kp = buttons & (1 << ((r * 4)+ c) ) ;
			...
			bitWrite(bitMap[r], c, Kp );  // keypress is active low so invert to high.
		}
	  }
}
